        ...
        | None => {
            ...
            switch(typ){
                    ...
                    switch(method) {
                        | "print_string" => {
                            ...
                            (("print_string", (0,0)),
                             ...
                             (Internal("IO.print_string"), (0,0)))
                        }
                        | "print_int" => {
                            // Add one Int parameter
                            (("print_int", (0,0)),
                             [("n", (0, 0))],    // Parameter name "n"
                             (Internal("IO.print_int"), (0,0)))
                        }
                        | "read_string" => {
                            (("read_string", (0,0)), [], (Internal("IO.read_string"), (0,0)))
                        }
                        | "read_int" => {
                            (("read_int", (0,0)), [], (Internal("IO.read_int"), (0,0)))
                        }
                        | _ => {
                            find_method(prog, "Object", method)
                        }
                ...
                | "String" => {
                    switch(method){
                        | "concat" => {
                            // (("concat", (0,0)), [("String", (0, 0))], (Internal("String.concat"), (0,0)))
                            (("concat", (0,0)), [("other", (0,0))], (Internal("String.concat"), (0,0)))

                        }
                        | "length" => {
                            /* (("length", (0,0)), [], (
                                ("String.length"), (0,0))) */
                                (("length", (0,0)), [], (Internal("String.length"), (0,0)))

                        }
                        | "substr" => {
                            // (("substr", (0,0)), [("Int", (0, 0)), ("Int", (0, 0))], (Internal("String.substr"), (0,0)))
                            (("substr", (0,0)), [("start", (0,0)), ("length", (0,0))], (Internal("String.substr"), (0,0)))
                        }
                        | _ => {
                            find_method(prog, "Object", method)
                        }
                ...
                | "Object" => {
                    switch(method){
                        | "abort" => {
                            ...
                            (("abort", (0,0)), [], (Internal("Object.abort"), (0,0)))
                        }
                        | "copy" => {
                            // produces a shallow copy of the object.1 This method will fail for an Array
                            (("copy", (0,0)), [], (Internal("Object.copy"), (0,0)))
                        }
                        | "get_type" => {
                            // returns a String with the name of the class of the object
                            (("get_type", (0,0)), [], (Internal("Object.get_type"), (0,0)))
                        }
                        // | "is_a" => {
                        //     //  returns true if the class name, t, is in the inheritance graph of the object or false otherwise
                        //     (("is_a", (0,0)), [], (Internal("Object.is_a"), (0,0)))
                        // }
                        | "is_a" => {
                        (("is_a",(0,0)), [("t",(0,0))], (Internal("Object.is_a"), (0,0)))
                        }
                        | _ => {
                            find_method(prog, "Object", method)
                        }
...
let rec evaluate_expression = ((prog: program),
    ...
    switch(expval) {
        /* | DynamicDispatch(target, method, arguments) => {
            ...
            switch(target_v) {
                | Dyn(t, members) => {
                    let (_, params, body) = find_method(prog, t, method_name);

                    // First evaluate all arguments
                    let rec evaluate_args = (args, env, store) => {
                        switch(args) {
                            | [] => ([], env, store)
                            | [arg, ...rest] => {
                                let (arg_v, env1, store1) = evaluate_expression(prog, so, env, store, arg);
                                let (rest_v, env2, store2) = evaluate_args(rest, env1, store1);
                                ([arg_v, ...rest_v], env2, store2)
                            }
                        }
                    };

                    let (args_v, e_2, s_2) = evaluate_args(arguments, e_1, s_1);

                    // Verify parameter and argument counts match
                    if (List.length(params) != List.length(args_v)) {
                        runtime_error(line, col, Printf.sprintf(
                    ...
                    evaluate_expression(prog, target_v, method_env, method_store, body)
                }
                | _ => runtime_error(line, col, "dynamic dispatch not allowed with this object")
            }
        }
        ...
        | StaticDispatch(target, cls, method, arguments) => {
            ...
            let (args_values, e_2, s_2) = List.fold_left(
                ((acc_vals, acc_env, acc_store), arg) => {
                    let (arg_v, new_env, new_store) = evaluate_expression(prog, so, acc_env, acc_store, arg);
                    ([arg_v, ...acc_vals], new_env, new_store)
                },
                ([], e_1, s_1),
                arguments
            );

            let method_def = find_method(prog, class_name, method_name);
            let (_, params, body) = method_def;

            // Create new environment with parameters bound to argument values
        ...
        | SelfDispatch(method, arguments) => {
            ...
            // Use self object (so) as target
            switch(so) {
                | Dyn(class_name, _) => {
                    // Similar to DynamicDispatch but using self object
                    let method_def = find_method(prog, class_name, method_name);
                    let (_, params, body) = method_def;

                    // Evaluate arguments
                    let (args_values, e_1, s_1) = List.fold_left(
                        ((acc_vals, acc_env, acc_store), arg) => {
                    ...
                    evaluate_expression(prog, so, method_env, method_store, body)
        ...
        /* | DynamicDispatch(target, method, arguments) => {
            ...
            let rec eval_args = (args, env, store, acc) => {
                switch(args) {
                    | [] => (List.rev(acc), env, store)  // Reverse to maintain order
                    | [arg, ...rest] => {
                        let (arg_v, env1, store1) = evaluate_expression(prog, so, env, store, arg);
                        eval_args(rest, env1, store1, [arg_v, ...acc])
                    }
                }
            };
            ...
            switch(target_v) {
                | Dyn(t, members) => {
                    let (_, params, body) = find_method(prog, t, method_name);
                    let (args_v, e_2, s_2) = eval_args(arguments, e_1, s_1, []);

                    // Create new locations for parameters
                    let rec bind_params = (plist, args, env, store) => {
                        switch(plist, args) {
                            | ([], []) => (env, store)
                            | ([(pname, _), ...prest], [arg, ...arest]) => {
                                let loc = newloc();
                                let new_env = StringMap.add(pname, loc, env);
                                let new_store = LocationMap.add(loc, arg, store);
                                bind_params(prest, arest, new_env, new_store)
                            }
                            | _ => runtime_error(line, col,
                                Printf.sprintf("Wrong number of arguments for %s", method_name))
                        }
                    };

                    // Bind parameters in new environment
                    let (method_env, method_store) = bind_params(params, args_v, emptyEnv, s_2);

                    // Add member bindings to environment
                    ...
                    evaluate_expression(prog, target_v, final_env, method_store, body)
                }
        ...
        | DynamicDispatch(target, method, arguments) => {
            ...
            // Find method
            let (_, params, body) = find_method(prog, typ, method_name);
        ...
        | StaticDispatch(target, cls, method, arguments) => {
            ...
            switch(target_v) {
                | Dyn(_, members) => {  // Note: we use class_name, not runtime type
                    let (_, params, body) = find_method(prog, class_name, method_name);
        ...
        | SelfDispatch(method, arguments) => {
            ...
            switch(so) {
                | Dyn(class_name, members) => {
                    ...
                    // Create method environment with member bindings
                    let base_env = List.fold_left(
                        (env, (name, loc)) => StringMap.add(name, loc, env),
                        emptyEnv,
                        members
        ...
        /* | Int(value, loc) => {
            // same thing here with the loc, loc instead of loc
            let(locc, _) = loc
            switch(LocationMap.find_opt(locc, s)) {
                |Some(storedval) => {
                    (storedval, e, s);
                }
                | _ => {
                    failwith("Expected an int location, but got something else.");
                }
            }
        } */
        ...
        /* | While(guard, body) => {
            ...
            switch(guard_v) {
                ...
                | _ => {
                    runtime_error(line, col, "guard needs to evalate to bool");
                }
            }
        } */
        | While(guard, body) => {
        ...
        | LetInit(target, expr) => {
            ...
            let (name, _) = target;
            let loc = newloc();
            let new_env = StringMap.add(name, loc, e_1);
            let new_store = LocationMap.add(loc, init_val, s_1);
            (init_val, new_env, new_store)
        }
        ...
        | ArrayAccess(array_expr, index_expr) => {
            ...
            switch (array_v, index_v) {
                | (Array(size, arr), Int(idx)) =>
                ...
                | (Array(_, _), _) =>
                    runtime_error(line, col, "Runtime error: array index must be an integer")
                | _ =>
                    runtime_error(line, col, "Runtime error: not an array")
            }
        }
            ...
            switch (array_v, index_v) {
                | (Array(size, arr), Int(idx)) =>
                    ...
                    if (index < 0) {
                        ...
                        new_arr[index] = value_v;
                        (Array(size, new_arr), e_3, s_3)
                    }
                | (Array(_, _), _) =>
                    runtime_error(line, col, "Runtime error: array index must be an integer")
                | _ =>
        ...
        | ArrayAssign(array_expr, index_expr, value_expr) => {
            ...
            // Find the actual array in the store if it's not directly an array
            let actual_array = switch(array_v) {
                | Array(size, arr) => Some((size, arr))
                | _ =>
                    // Try to find it in the current environment/store
                    ...
                    switch(index_v) {
                        | Int(idx) => {
                            ...
                            if (index < 0) {
                                ...
                                let new_arr = Array.copy(arr);
                                new_arr[index] = value_v;
                                let new_array = Array(size, new_arr);

                                // If original was from environment, update the store
                                if (array_v != Array(size, arr)) {
                                    let array_loc = StringMap.find("population_map", e_3);
                                    let new_store = LocationMap.add(array_loc, new_array, s_3);
                                    (new_array, e_3, new_store)
                                } else {
                                    (new_array, e_3, s_3)
                                }
                            }
        ...
        | ArrayAccess(array_expr, index_expr) => {
            ...
            // Similar to ArrayAssign, first find the actual array
            let actual_array = switch(array_v) {
        ...
        | Block(expressions) => {
            // Evaluate each expression in sequence, keeping track of environment and store
            List.fold_left(
            ...
            switch(internal_name){
                ...
                | "Object.copy" => {
                    switch(so) {
                        ...
                        // t - class name, for identifying the class type
                        | Dyn(t, members) => {
                            let new_members = List.map(
                                // map name and location from members
                                // to name, newloc in new_members
                                ((name, loc)) => (name, newloc()),
                                members
                            );
                            // extract the new value, same class name, new members
                            let new_value = Dyn(t, new_members);
                            // update the store
                            let new_store =
                                 List.fold_left(
                                 // we're accumulating the store, each member has its own value
                                 (store_acc, (name, new_loc)) =>{
                                     switch(LocationMap.find_opt(new_loc, s)) {
                                         | Some(value_at_loc) => LocationMap.add(new_loc, value_at_loc, store_acc)
                                         | None => store_acc  // If the location is not found, skip it
                                     }
                                 },
                                     s,
                                     new_members
                                ...
                                /*
                                List.fold_left2(
                                    (store_acc, (old_name, old_loc), (new_name, new_loc)) => {
                                        switch(LocationMap.find_opt(old_loc, s)) {
                                        | Some(value_at_loc) => LocationMap.add(new_loc, value_at_loc, store_acc)
                                        | None => store_acc
                                        }
                                    },
                                ...
                                (new_value, e, new_store);
                        }
                        | _ => {
                            runtime_error(line, col, "copy only supported for Objects");
                        }
                    }
                ...
                /* | "Object.get_type" => {
                        switch(so) {
                            | Dyn(t, _) => {
                                (String(String.length(t), t), e, s);
                            }
                            | _ => {
                                runtime_error(line, col, "get_type available only for object types");
                            }
                        }
                    } */
                    | "Object.get_type" => {
                ...
                | "IO.read_int" => {
                    ...
                    // Don't throw errors for invalid input, just return 0
                    let num = (
                        try(Int64.of_string(String.trim(input))) {
                            | _ => Int64.zero  // Return 0 for any invalid input including garbage
                        }
                    );
                    (Int(num), e, s)
                }
                ...
                | "IO.read_string" => {
                    // Just return whatever input we get, even if it's garbage or huge
                    let input = switch(input_line(stdin)) {
                        | value => value
                        | exception End_of_file => ""
                    };
                    (String(String.length(input), input), e, s)
                }
        ...
        // | _ => runtime_error(line, col, "Unhandled expression type")
    }
};
