...
and expr_value =
    ...
    | BoolOp(expression, expression, string) // and, or
    | Negate(expression)
    | Not(expression)
...
let rec json_to_expr_val = (json) => {
    ...
    switch(typ) {
        ...
        | "string" => {
            let str_value = to_string(member("value", json));
            // Only handle \n and \t escapes, preserve everything else
            let processed_value =
                str_value
                |> Str.global_replace(Str.regexp("\\\\n"), "\n")
                |> Str.global_replace(Str.regexp("\\\\t"), "\t");
            String(processed_value, json_to_loc(json))
        }

        | "int" => {
            let int_value = to_int(member("value", json));
            let int_value = Int64.of_int(int_value);
            Int(int_value, json_to_loc(json)); // Passing the location from the JSON
        }
        | "let" => {
        ...
        | "bool" => {
            Bool(to_bool(member("value", json)))
        }
        | "negate" => {
            Negate(json_to_expression(member("body", json)))
        }
        | "not" => {
            Not(json_to_expression(member("body", json)))
        }
        | "block" => {
            Block(
        ...
        | "number" => {
            let value = to_int(member("value", json));
            Int(Int64.of_int(value), json_to_loc(json))
        }
